...
sudo apt-get install -y build-essential vim curl python-software-properties wget pv > /dev/null 2>&1
...
sudo pecl install -f mongo > /dev/null 2>&1

echo "extension=xhprof.so" >> /etc/php5/cli/php.ini
